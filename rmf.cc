...
RMF::update() {
  ...
  std::vector<Vector3D> der; curve_->eval(0.0, 1, der);
...
RMF::rotationMatrix(const Vector3D &u, double theta) {
  static std::array<std::array<double, 3>, 3> m;
...
RMF::rotateFrame(Frame &f, double angle) {
  const std::array<std::array<double, 3>, 3> &r = rotationMatrix(f.d, angle);
...
RMF::nextFrame(const Frame &prev, double u) const {
  std::vector<Vector3D> der; curve_->eval(u, 1, der);
  der[1].normalize();
